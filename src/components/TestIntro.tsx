...
export const TestIntro: FC<TestIntroProps> = ({ onStart, onExploreCatalog }) => {
  return (
    <div className="w-full max-w-3xl mx-auto px-4 py-8 sm:py-16 flex flex-col items-center text-center">
      ...
      {/* Subtitle */}
      <p className="text-muted-foreground text-sm sm:text-base max-w-xl mb-12 leading-relaxed font-normal">
        일반적인 설문은 정제된 답변만 기록합니다. <br className="hidden sm:inline" />
        클릭하기 전의 머뭇거림, 선택지를 바꾼 고민의 시간, 커서의 궤적을 통해 당신의 본능적 성향과 사회적 페르소나를
        확인해보세요.
      </p>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 w-full mb-14 text-left">
        <Card className="group p-6 rounded-2xl bg-card/60 dark:bg-slate-900/50 hover:bg-card dark:hover:bg-slate-900/90 border-border dark:border-slate-800/70 hover:border-border dark:hover:border-slate-600/80 transition-all duration-300 ease-out hover:-translate-y-1 hover:shadow-xl flex flex-col justify-between cursor-default">
          <div>
            ...
            <p className="text-xs text-muted-foreground leading-relaxed font-normal transition-colors">
      ...
      <div className="flex flex-col sm:flex-row items-center gap-3.5">
        <Button
          size="lg"
          onClick={onStart}
          className="group relative px-8 py-4 text-sm sm:text-base font-semibold rounded-full hover:shadow-lg transition-all duration-200"
        >
        ...
        {onExploreCatalog && (
          <Button
            ...
            className="px-6 py-4 text-sm font-medium rounded-full"
          >
