...
export function analyzeBehaviorAndMBTI(logs: QuestionBehaviorLog[]): FullAnalysisResult {
  ...
  const indecisivenessIndex = Math.min(
    100,
    Math.round((totalAnswerChanges * 18 + (totalDirectionChanges / (logs.length || 1)) * 5 + (totalTestDuration / logs.length / 1000) * 3))
  );

  // 4. Determine Behavior Persona
  ...
  // 5. Detect Persona Gap (초기 선택 vs 최종 수정)
  const personaGap = detectPersonaGap(logs, logMap);

  // 6. Extract Top Dilemmas (가장 치열하게 고민한 질문 Top 3)
...
function calculateDimension(
  ...
  let totalScore = 0; // positive is posType, negative is negType
  let totalChanges = 0;
  let totalDwell = 0;
  ...
  // Normalized score: 0 to 100% towards winner
  ...
  // Certainty Score: high if strong answers and low change count/hesitation
  const avgDwellSec = totalDwell / dimQuestions.length / 1000;
  const strengthFactor = Math.abs(totalScore) / maxPossible; // 0 to 1
  const penalty = totalChanges * 12 + (totalHesitation / dimQuestions.length) * 0.4 + (avgDwellSec > 10 ? 15 : 0);
